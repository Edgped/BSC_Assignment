...
```{r FI_histogram, fig.cap="Histogram of Frustration Index for all PDB files. This plot shows the frequency of the Frustration Indexes found in all the PDBs from the list."}    
...
<span class="orange_accent">
<b>COMMENT</b>: The Figure above is that extended to the right (Frustration index values of ~ 8.0) because of a single PDB (2M5C), and that extended to the left (Frustration index values of ~ -4.5) mostly because of a pair of PDBs (2K1S & 6EMR). 
<br>
Only if the ["INDIVIDUAL FRUSTRATION INDEX HISTOGRAMS" chunk](#IndivFrstHist) has been executed, those individual Frustration Index histograms can be visualized here: [2M5C](./PLOTS/2M5C_A_FrstIndex_Histogram.jpg), [2K1S](./PLOTS/2K1S_A_FrstIndex_Histogram.jpg) & [6EMR](./PLOTS/6EMR_A_FrstIndex_Histogram.jpg).
</span>
...
```{r AverageFI_perResidue_histogram, fig.cap="Frustration per residue histogram for all PDB files. This plot shows the Average Frustration Index for each of the 20 amino acids across all the PDBs from the list."}
...
```{r AA_Freq, fig.cap="Frequency of each Amino Acid in all PDB files. This plot shows the frequency of the 20 amino acids across all the PDBs from the list."}
