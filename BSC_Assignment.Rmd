...
author: "Edgar Chacón (edgarpedro.chacon01@estudiant.upf.edu)"
...
This file corresponds to the assignment asked for the application to the "<span class="orange_accent">Computational Biology Group</span>" at <span class="orange_accent">Barcelona Supercomputing Center</span> (<b class="green_accent">BSC</b>).
<br>
<br>
...
To calculate the single residue frustrations for each PDB file (`r length(pdb_ids)`), first I created a <span class="green_accent">`RESULTS folder`</span> where the output files will be stored. 
<br>
...
results_folder <- "/home/rstudio/myfiles/BSC_Assignment/RESULTS/"
...
dir_frustration(PdbsDir = "/home/rstudio/myfiles/BSC_Assignment/PDBs/",
                OrderList = NULL,
                Chain = pdb_chain,
                Mode = "singleresidue",
...
Finally, as requested by Gonzalo, I add here the code I previously used to generate a histogram for the frustration results of each one of the PDB files individually.
<br>
...
plots_folder <- "/home/rstudio/myfiles/BSC_Assignment/PLOTS/"
...
for (pdb_id in pdb_ids) {
  ...
  if (file.exists(file_path)) {
    ...
    p <- ggplot(individual_data, aes(x = FrstIndex)) +
      geom_histogram(binwidth = 0.1, fill = my_green, alpha = 0.6, color = alpha("white", 0.5)) +
    ...
    output_file <- file.path(plots_folder, paste0(pdb_id, "_", pdb_chain, "_FrstIndex_Histogram.png"))
...
All individual frustration index histograms can be accessed on the <span class="green_accent">`PLOTS folder`</span>.
...
<br>
Therefore, to save resources, instead of starting from scratch with a count of amino acids extracted from the PDB files (too much computationally demanding in terms of execution time), I decided to utilize the data resulting from the calculation of frustrations per single residue, which I already had combined.
<br>
